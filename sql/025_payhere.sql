-- Payment & Order Management System (v10), Phase 6 — PayHere card payments.
--
-- Only change: widen create_order_atomic's payment-method check to accept
-- 'payhere' (it currently rejects it outright). No other schema change
-- needed — payments.transaction_id (-> PayHere's payment_id) and
-- payments.raw_gateway_response (full callback payload) already exist
-- from Phase 1. Same signature as the live sql/024 version, so this is a
-- clean `create or replace`, not a new overload.
