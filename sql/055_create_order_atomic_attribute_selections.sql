-- Same signature as sql/052 — only the second item loop's insert gains one
-- more column. attribute_selections is client-supplied jsonb, trusted
-- as-is (display/reference only, no price/stock derivation from it) —
-- the same trust level already given to p_notes (free text) elsewhere in
-- this function. Omitted/null behaves exactly like every order placed
