-- Checkout Redesign (v12), Phase 0 — fixes a real, live pricing bug: the
-- discounted delivery rate was keyed on shipping DISTRICT ('Colombo'),
-- which covers the entire Colombo District (Colombo city zones *and*
-- Dehiwala, Moratuwa, Kaduwela, Homagama, Maharagama, etc.) — all of which
-- were being undercharged Rs.145 versus what AnnouncementBar already (and
-- correctly) advertises: "Colombo 1-15" is the CITY postal zone, not the
-- district. Fee now keys off shipping CITY matching a Colombo 01-15 zone,
-- tolerant of format (case, optional space/dash, optional leading zero) —
-- kept in sync by hand with lib/shipping-rates.ts's isColomboZone regex.
--
-- Same parameter list as the currently-live version (sql/027) — a
-- logic-only change inside the function body, so this is a clean
-- `create or replace`, no DROP FUNCTION needed (only required when the
-- parameter list itself changes shape, per the sql/022/sql/026 incidents).
...
begin
  ...
  -- Only a Colombo 01-15 CITY zone gets the discounted rate — see the
