-- Fixes a live bug confirmed by calling create_order_atomic directly via
-- the REST API (both as a guest and as a logged-in test user, same error
-- both times): "record \"v_coupon\" is not assigned yet" (SQLSTATE 55000).
--
-- v_coupon is a generic `record`, only ever assigned inside
-- `if p_coupon_code is not null and trim(p_coupon_code) <> '' then ...`.
-- When no coupon code is given (the common case), that SELECT INTO never
-- runs, so v_coupon is never assigned ANY row — not even a zero-row one.
-- The later unconditional `if v_coupon.id is not null then` then throws,
-- because accessing a field on a truly unassigned record variable is a
-- hard runtime error in PL/pgSQL (unlike a %rowtype variable, which would
-- have all-null fields even unassigned). This has likely been broken
-- since coupons shipped (sql/026) for every coupon-less order — testing
-- during that phase apparently always exercised the coupon path itself.
--
-- Fix: a plain boolean flag, set true only when a coupon was actually
-- validated, checked instead of `v_coupon.id is not null`. This sidesteps
-- the whole "record not assigned" class of bug rather than patching just
-- this one spot with %rowtype (which would still be fragile if another
-- such variable is added later).
--
-- Logic-only change, same 19-arg signature as sql/034 — a clean
...
begin
  ...
  -- Only a Colombo 01-15 CITY zone gets the discounted rate.
