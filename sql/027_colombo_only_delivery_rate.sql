-- Cart Page Redesign (v11), Phase 0 — confirmed pricing change: the cheap
-- delivery tier narrows from "Western Province" (Colombo/Gampaha/Kalutara)
-- to Colombo district only, matching the AnnouncementBar's copy (which was
-- already correct) instead of the wider definition /shipping and this
-- function previously used. Gampaha/Kalutara move from Rs.255 to Rs.400.
-- See lib/shipping-rates.ts for the single JS-side source of truth these
-- literal values must be kept in sync with.
--
-- Same parameter list as the currently-live version (sql/026) — a
-- logic-only change inside the function body, so this is a clean
-- `create or replace`, no DROP FUNCTION needed (that's only required when
-- the parameter list itself changes shape, per the sql/022/sql/026
-- incidents).
...
begin
  ...
  -- Colombo district only gets the discounted rate — see the header comment.
  if p_delivery_method = 'pickup' then
    v_delivery_fee := 0; v_street := 'Salawatta Road'; v_city := 'Wellampitiya'; v_district := 'Colombo';
  else
    v_delivery_fee := case when p_shipping_district = 'Colombo' then 255 else 400 end;
