-- Homepage coupon display needs title/description text the code itself
-- never had (it only ever needed to validate and price), plus a cap on
-- percentage discounts admins can optionally set. Extends the existing
-- coupons table from sql/020/026 rather than duplicating it.

alter table public.coupons
  add column title text,
  add column description text,
  add column max_discount_amount numeric(10,2) check (max_discount_amount is null or max_discount_amount >= 0);

-- Safety net: an earlier version of this migration mistakenly
-- CREATE-OR-REPLACEd create_order_atomic against a STALE 17-arg copy of
-- the function (missing everything sql/027 through sql/038 added since —
-- guest checkout, idempotency, source_address_id, postal-code delivery
-- pricing). Because the arg list didn't match the real, live 19-arg
-- version, Postgres treated it as a distinct overload rather than
-- replacing anything, which is why the run failed at the final ambiguous
-- GRANT rather than silently corrupting checkout — but the stray 17-arg
-- overload may still exist depending on how much of that failed script
-- committed. This drops it if present; a no-op if it never landed.
drop function if exists public.create_order_atomic(
  text, text, text, text, text, text, text, text, text, text, text, text,
  jsonb, numeric, text, text, text
);

-- Same 19-arg signature as sql/038 (the actual current version — verified
-- by reading it directly, not assumed) — only the percentage branch's
-- formula changes (capped at max_discount_amount when set), so create or
-- replace is safe here with no DROP needed for the real overload.
...
begin
  ...
  if p_coupon_code is not null and trim(p_coupon_code) <> '' then
    ...
    -- Only the percentage branch changes from sql/038: capped at
