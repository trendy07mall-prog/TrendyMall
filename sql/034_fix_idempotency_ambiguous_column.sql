-- Fixes a live bug introduced in sql/032: the idempotency-replay check
-- added to create_order_atomic read a bare `order_number` from
-- `public.orders`, which collides with the function's own implicit
-- RETURNS TABLE output variable of the same name (returns table (order_id
-- uuid, order_number text)) — exactly the same ambiguity class as the
-- original "column reference order_number is ambiguous" bug (sql/024),
-- just in a new location that never got the same table-aliasing fix.
-- Because every checkout submission now always sends a non-empty
-- idempotency key (CheckoutForm.tsx), this fired on every single order
-- attempt since sql/032 shipped.
--
-- Logic-only change inside the function body, same 19-arg signature as
-- sql/033 — a clean `create or replace`, no DROP FUNCTION needed.
...
begin
  ...
  -- re-running any stock reduction or coupon redemption. Fixed: `o.`
  -- alias on every column read, since `order_number` bare would collide
  -- with this function's own implicit RETURNS TABLE variable of the
  -- same name (the actual bug — see this migration's header comment).
  ...
  -- customer is a no-op for guests (v_user_id is null).
