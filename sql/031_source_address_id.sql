-- Checkout Redesign (v12), Phase 2 — closes the Phase 1 deferral: links an
-- order's shipping_addresses snapshot back to the customer_addresses row it
-- came from, for TRACEABILITY ONLY. Never read as an authority for what an
-- order actually shipped to — shipping_addresses stays the snapshot of
-- record, exactly as before; editing or deleting the source
-- customer_addresses row (even soft-deleting it) must never alter this or
-- any other past order, which is why this is `on delete set null`, not
-- cascade.
alter table public.shipping_addresses
  add column source_address_id uuid references public.customer_addresses(id) on delete set null;
...
-- (matching the exact 17-arg type list of the currently-live version, sql/029)
-- before CREATE OR REPLACE — a plain `create or replace` would leave both
-- overloads live and make the next unqualified GRANT/call ambiguous.
drop function if exists public.create_order_atomic(
  text, text, text, text, text, text, text, text, text, text, text, text, jsonb, numeric, text, text, text
);
