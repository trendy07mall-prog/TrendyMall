-- Adds the coupon CODE (not just the discount amount, already returned by
-- all three of these as `discount`) to every customer-facing order-read
-- RPC. There's no coupon_code column on orders -- the code lives on
-- coupons.code, joined via coupon_redemptions (unique per order_id).
-- Additive jsonb key only, same return type on all three, no DROP FUNCTION
-- needed. Each function is security definer, so this subquery bypasses RLS
-- on coupon_redemptions/coupons entirely -- deliberate: a customer viewing
-- their own past order should still see which coupon they used even if
-- that coupon has since expired or been deactivated (coupons' own RLS
-- would otherwise hide it from a plain client-side join).
